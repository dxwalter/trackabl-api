...
describe("AuthController (e2e)", () => {
  ...
  it("Map Free Subscription plan to nigerian market", () => {
  ...
  it("Map Subscription plan to US market", () => {
    ...
    return request(app.getHttpServer())
      ...
      .expect(function (res) {
        ...
        priceIds.priceId3 = res.body.data.id;
  ...
  it("Map Free Subscription plan to US market", () => {
    const getPlatinum = plans.filter(
      (plan) => plan.name.toLowerCase() === "free"
    );
  ...
  it("Get user past subscriptions [SUCCESS]", () => {
    return request(app.getHttpServer())
      .get("/subscription")
      .set("Authorization", `Bearer ${userToken}`)
      .set("Accept", "application/json")
  ...
  it("3", () => {
    return request(app.getHttpServer())
      .delete("/subscription/price/" + priceIds.priceId3)
      .set("Authorization", `Bearer ${adminToken}`)
      .set("Accept", "application/json")
      .expect(function (res) {
        if (!("message" in res.body)) {
